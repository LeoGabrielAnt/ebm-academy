...
function initializeAcademy() {
  ...
  updateProgress();
  if(window.EBMI18N && window.EBMI18N.updateStaticUi) window.EBMI18N.updateStaticUi();
...
function renderTrainingMenu(track='hvac') {
  const tr = (key, fallback) => window.EBMI18N && window.EBMI18N.t ? window.EBMI18N.t(key, fallback) : fallback;
  const title = document.querySelector('.sidebar h2');
  const menu = document.querySelector('.menu');
  const topTitle = document.querySelector('.topbar-left h2');
  if(!menu) return;
  if(track === 'eletrica') {
    if(title) title.textContent = tr('trackElectrical','TRILHA ELÉTRICA APLICADA A HVAC');
    if(topTitle) topTitle.textContent = tr('trainingTitleElectrical','EBM Academy • Elétrica');
    menu.innerHTML = `
      <a href="#" data-page="eletrica-home" class="active"><i class="fa-solid fa-bolt"></i><span>${tr('menuElectricalHome','Home Elétrica')}</span></a>
      <a href="#" data-page="eletrica01"><i class="fa-solid fa-plug-circle-bolt"></i><span>E01 • ${tr('menuFundamentals','Fundamentos')}</span></a>
      <a href="#" data-page="eletrica02"><i class="fa-solid fa-diagram-project"></i><span>E02 • ${tr('menuControls','Comandos')}</span></a>
      <a href="#" data-page="eletrica03"><i class="fa-solid fa-gauge-high"></i><span>E03 • ${tr('menuInstruments','Instrumentos')}</span></a>
      <a href="#" data-page="eletrica04"><i class="fa-solid fa-stethoscope"></i><span>E04 • ${tr('menuDiagnosis','Diagnóstico')}</span></a>
      <a href="#" data-page="eletrica-diagnostic-tool"><i class="fa-solid fa-toolbox"></i><span>${tr('menuTool','Ferramenta')}</span></a>
      <a href="#" data-page="eletrica-final-assessment"><i class="fa-solid fa-medal"></i><span>${tr('menuFinalAssessment','Avaliação Final')}</span></a>
    `;
  } else {
    if(title) title.textContent = tr('trackHvac','TRILHA HVAC CRÍTICO');
    if(topTitle) topTitle.textContent = tr('trainingTitleHvac','EBM Academy • HVAC Crítico');
    menu.innerHTML = `
      <a href="#" data-page="training-home" class="active"><i class="fa-solid fa-graduation-cap"></i><span>${tr('menuTrainingHome','Home Treinamento')}</span></a>
      <a href="#" data-page="modulo01"><i class="fa-solid fa-snowflake"></i><span>${tr('menuCycle','Ciclo Frigorífico')}</span></a>
      <a href="#" data-page="modulo02"><i class="fa-solid fa-temperature-high"></i><span>SH & SC</span></a>
      <a href="#" data-page="modulo03"><i class="fa-solid fa-wind"></i><span>${tr('menuPsychrometrics','Psicrometria')}</span></a>
      <a href="#" data-page="modulo04"><i class="fa-solid fa-box"></i><span>Self Contained DX</span></a>
      <a href="#" data-page="modulo05"><i class="fa-solid fa-microchip"></i><span>${tr('menuPrecisionSelfs','Selfs de Precisão')}</span></a>
      <a href="#" data-page="modulo06"><i class="fa-solid fa-fan"></i><span>${tr('menuPrecisionFanCoils','Fancoils de Precisão')}</span></a>
      <a href="#" data-page="modulo07"><i class="fa-solid fa-water"></i><span>${tr('menuChilledWater','Central Água Gelada')}</span></a>
      <a href="#" data-page="modulo08"><i class="fa-solid fa-circle-nodes"></i><span>${tr('menuPumpsValves','Bombas e Válvulas')}</span></a>
      <a href="#" data-page="modulo09"><i class="fa-solid fa-sitemap"></i><span>${tr('menuAutomation','Automação e BMS')}</span></a>
      <a href="#" data-page="modulo10"><i class="fa-solid fa-screwdriver-wrench"></i><span>${tr('menuTroubleshooting','Alarmes e Troubleshooting')}</span></a>
      <a href="#" data-page="tools"><i class="fa-solid fa-toolbox"></i><span>${tr('menuHvacTools','Ferramentas HVAC')}</span></a>
      <a href="#" data-page="hvac-final-assessment"><i class="fa-solid fa-award"></i><span>${tr('menuFinalAssessment','Avaliação Final')}</span></a>
    `;
  }
...
window.renderTrainingMenu = renderTrainingMenu;
...
async function loadPortalPage(page) {
  ...
  window.EBM_ACTIVE_PORTAL_PAGE = page;
  try {
    const res = await fetch(`modules/${page}.html`);
    if(!res.ok) throw new Error(page);
    container.innerHTML = await res.text();
    initializePageFeatures(page);
    if(window.EBMI18N && window.EBMI18N.afterPageLoad) window.EBMI18N.afterPageLoad(page, container);
  }
  catch(e) {
    const title = window.EBMI18N && window.EBMI18N.t ? window.EBMI18N.t('loadErrorTitle','Erro') : 'Erro';
    const text = window.EBMI18N && window.EBMI18N.t ? window.EBMI18N.t('loadErrorText','Não foi possível carregar:') : 'Não foi possível carregar:';
    container.innerHTML = `<section class="section"><div class="card"><h3>${title}</h3><p>${text} ${page}</p></div></section>`;
  }
}
async function loadTrainingPage(page) {
  const container = document.getElementById('app-content'); if (!container) return;
  window.EBM_ACTIVE_TRAINING_PAGE = page;
  try {
    const url = window.EBMI18N && window.EBMI18N.localizedModuleUrl ? window.EBMI18N.localizedModuleUrl(page) : `modules/${page}.html`;
    const res = await fetch(url);
    if(!res.ok) throw new Error(page);
    container.innerHTML = await res.text();
    setActiveMenu(page.split('/')[0]);
    initializePageFeatures(page);
    if(window.EBMI18N && window.EBMI18N.afterPageLoad) window.EBMI18N.afterPageLoad(page, container);
  }
  catch(e) {
...
window.loadTrainingPage = loadTrainingPage;
window.loadPortalPage = loadPortalPage;
...
function updateThemeButtons() { if(window.EBMI18N && window.EBMI18N.updateThemeLabels) { window.EBMI18N.updateThemeLabels(); return; } const isLight=document.body.classList.contains('light-mode'); ['portal-theme-toggle','training-theme-toggle'].forEach(id=>{ const b=document.getElementById(id); if(b) b.innerHTML = isLight ? '<i class="fa-solid fa-moon"></i><span>Modo Escuro</span>' : '<i class="fa-solid fa-sun"></i><span>Modo Claro</span>'; }); }
...
function unlockChecklist(status,btn) { const msg=window.EBMI18N&&window.EBMI18N.t?window.EBMI18N.t('checklistDone','Checklist concluído. Avaliação liberada.'):'Checklist concluído. Avaliação liberada.'; status.innerHTML=msg; status.classList.add('approved'); btn.disabled=false; btn.classList.remove('btn-disabled'); }
function lockChecklist(status,btn) { const msg=window.EBMI18N&&window.EBMI18N.t?window.EBMI18N.t('checklistPending','Conclua o checklist para liberar a avaliação.'):'Conclua o checklist para liberar a avaliação.'; status.innerHTML=msg; status.classList.remove('approved'); btn.disabled=true; btn.classList.add('btn-disabled'); }
